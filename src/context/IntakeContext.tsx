...
import { StudentStateModel, PersonaMode, ResolvedStateModel, ConflictOverrides } from '../types';
...
function resolveState(raw: StudentStateModel): ResolvedStateModel {
  ...
  return { intent: raw.intent, mode: raw.mode, friction, confidence, avoidance, scaffolding, values, rhythm, learning, accessibility, overrides };
}
...
function resolveConflicts(raw: StudentStateModel): ConflictOverrides {
  ...
  // Energy paradox: claims long bursts but low energy or hyperfocus/crash
  if (raw.workBurst === '60+' && (raw.avoidanceRoot === 'energy' || raw.accessNeeds.includes('hyperfocus'))) {
  ...
  // Autonomy vs need: prefers figuring it out but drained by mental/decision effort
  ...
  // Structure + low autonomy avoidance
  if ((raw.structurePref === 'fulls' || raw.structurePref === 'sf') && raw.avoidanceRoot === 'direction') {
    // Not a true conflict — both align. No override needed.
  }

  // Flexibility + exec function needs
  if ((raw.structurePref === 'fullf' || raw.structurePref === 'fs') &&
  ...
  // Social contradiction: social drains + helping others as capability builder
...
export const IntakeProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  return (
    <IntakeContext.Provider value={{ currentStep, totalSteps, state, nextStep, prevStep, updateState, setStep, getResolvedState }}>
      {children}
