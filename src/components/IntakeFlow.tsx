...
// Added missing User and Calendar icons to the imports
import {
  ChevronLeft, ChevronRight, Sparkles, Brain, Zap, Heart,
  ShieldCheck, RefreshCw, Clock, BookOpen, Accessibility, AlertTriangle, ArrowRight, Info, Loader2,
  User, Calendar
} from 'lucide-react';
...
export const IntakeFlow: React.FC<IntakeFlowProps> = ({ onComplete }) => {
  const { currentStep, nextStep, prevStep, state, updateState, setStep, getResolvedState } = useIntake();
  const { signIn, isLoggingIn, session } = useSession();
  ...
  const renderStep = () => {
    switch (currentStep) {
        ...
        return (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center max-w-xl mx-auto">
            <div className="w-20 h-20 bg-emerald-100 rounded-3xl flex items-center justify-center mx-auto mb-8 text-emerald-600">
              {/* Fix: Changed size(40) to size={40} */}
              <Sparkles size={40} />
            </div>
            ...
            <button onClick={nextStep} className="px-10 py-4 bg-stone-900 text-white rounded-full font-medium hover:bg-stone-800 transition-colors shadow-lg shadow-stone-200">
              Help me help you
            </button>
          </motion.div>
        );
      case 1:
        return (
          <QuestionStep title="The Big Why" subtitle="What do you want help with right now?" allowMultiple
            options={[
            ...
            selectedValues={state.intent} onSelect={(val) => handleToggleValue('intent', val, true)}
          />
        );
      case 2:
        return (
          <QuestionStep title="Intrinsic Core" subtitle="What drains you faster than it should?"
            options={[
            ...
            selectedValues={state.drains} onSelect={(val) => handleToggleValue('drains', val)}
          />
        );
      case 3:
        return (
          <QuestionStep title="Feeling Capable" subtitle="What makes you feel competent pretty quickly?"
            options={[
            ...
            selectedValues={state.capabilities} onSelect={(val) => handleToggleValue('capabilities', val)}
          />
        );
      case 4:
        return (
          <QuestionStep title="Avoidance Pattern" subtitle="When you avoid something, what's usually underneath?"
            options={[
            ...
            selectedValues={state.avoidanceRoot} onSelect={(val) => handleToggleValue('avoidanceRoot', val)}
          />
        );
      case 5:
        return (
          <QuestionStep title="Structure vs Flexibility" subtitle="What's your preferred way to work?"
            options={[
            ...
            selectedValues={state.structurePref} onSelect={(val) => handleToggleValue('structurePref', val)}
          />
        );
      case 6:
        return (
          <QuestionStep title="A Wasted Day" subtitle='When does a day feel "wasted" to you?'
            options={[
            ...
            selectedValues={state.valueAlignment} onSelect={(val) => handleToggleValue('valueAlignment', val)}
          />
        );
      case 7:
        return (
          <div className="max-w-2xl mx-auto">
            <header className="mb-10 text-center">
              <div className="w-14 h-14 bg-amber-100 rounded-2xl flex items-center justify-center mx-auto mb-6 text-amber-600"><Clock size={28} /></div>
              <h2 className="text-3xl font-serif font-light text-stone-800 mb-3">Energy & Rhythm</h2>
              <p className="text-stone-500 text-lg font-light leading-relaxed">This helps me suggest better timing.</p>
            </header>
            <SubQuestion title="Best focus time" options={[{id:'morning',label:'Morning'},{id:'afternoon',label:'Afternoon'},{id:'night',label:'Night'},{id:'unpredictable',label:'Unpredictable'}]} selected={state.focusTime} onSelect={(v)=>updateState({focusTime:v})} />
            <SubQuestion title="Ideal work burst" options={[{id:'15-25',label:'15-25 minutes'},{id:'30-45',label:'30-45 minutes'},{id:'60+',label:'60+ minutes'}]} selected={state.workBurst} onSelect={(v)=>updateState({workBurst:v})} />
            <SubQuestion title="Recovery style" options={[{id:'rest',label:'Rest & stillness'},{id:'movement',label:'Movement'},{id:'distraction',label:'Distraction'},{id:'switching',label:'Switching tasks'}]} selected={state.recovery} onSelect={(v)=>updateState({recovery:v})} />
          </div>
        );
      case 8:
        return (
          <div className="max-w-2xl mx-auto">
            <header className="mb-10 text-center">
              <div className="w-14 h-14 bg-blue-100 rounded-2xl flex items-center justify-center mx-auto mb-6 text-blue-600"><BookOpen size={28} /></div>
            ...
            <SubQuestion title="I learn best by" options={[{id:'explanation-first',label:'Explanation first'},{id:'example-first',label:'Example first'},{id:'trying-first',label:'Trying it first'}]} selected={state.learnStyle} onSelect={(v)=>updateState({learnStyle:v})} />
            <SubQuestion title="Feedback style" options={[{id:'encouraging',label:'Encouraging'},{id:'direct',label:'Direct'},{id:'minimal',label:'Minimal'}]} selected={state.feedbackPref} onSelect={(v)=>updateState({feedbackPref:v})} />
            <SubQuestion title="Reminders" options={[{id:'gentle',label:'Gentle nudges'},{id:'firm',label:'Firm check-ins'},{id:'only-when-asked',label:'Only when I ask'}]} selected={state.reminderPref} onSelect={(v)=>updateState({reminderPref:v})} />
          </div>
        );
      case 9:
        return (
          <QuestionStep title="Accessibility & Friction" subtitle="You don't need a diagnosis for this." allowMultiple
            options={[{id:'starting',label:'Trouble starting tasks'},{id:'remembering',label:'Trouble remembering steps'},{id:'sensory',label:'Sensory overwhelm'},{id:'hyperfocus',label:'Hyperfocus then crash'},{id:'anxiety',label:'Anxiety around performance'},{id:'none',label:'None / prefer not to say'}]}
            selectedValues={state.accessNeeds} onSelect={handleAccessToggle}
          />
        );
      case 10: {
        const resolved = getResolvedState();
        const activeOverrides = Object.values(resolved.overrides).filter(o => o.active);
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl mx-auto">
            <div className="bg-white rounded-[2.5rem] p-10 shadow-xl shadow-stone-200/50 border border-stone-100 mb-10">
              <div className="flex items-center gap-4 mb-8">
                <div className="w-14 h-14 bg-emerald-500 rounded-2xl flex items-center justify-center text-white"><ShieldCheck size={30} /></div>
                <div><h3 className="text-2xl font-serif text-stone-800">Your Sensei Profile</h3><p className="text-stone-400 font-light">The model is established.</p></div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                <SummaryItem icon={<Zap size={20} />} title="Mode" value={resolved.mode} />
                <SummaryItem icon={<Brain size={20} />} title="Primary Friction" value={label(resolved.friction.primary)} />
                <SummaryItem icon={<Heart size={20} />} title="Confidence Loop" value={label(resolved.confidence.builder)} />
                <SummaryItem icon={<RefreshCw size={20} />} title="Avoidance Reframe" value={label(resolved.avoidance.root_cause)} />
              </div>

              {/* Coach's Notes for Transparency */}
              {activeOverrides.length > 0 && (
                <div className="mb-8 p-6 bg-stone-50 rounded-2xl border border-amber-100">
                  <div className="flex items-center gap-2 mb-3 text-amber-700 font-bold text-xs uppercase tracking-widest">
                    <Info size={14} />
                    Coach's Strategic Notes
                  </div>
                  <ul className="space-y-3">
                    {activeOverrides.map((o, idx) => (
                      <li key={idx} className="text-sm text-stone-600 leading-relaxed font-light flex gap-3">
                        <div className="w-1 h-auto bg-amber-200 shrink-0" />
                        {o.note}
                      </li>
                    ))}
                  </ul>
              ...
              <div className="p-6 bg-emerald-50 rounded-2xl border border-emerald-100">
                <p className="text-emerald-900 font-light italic leading-relaxed text-sm">
                  "I've mapped your friction points. We'll start with {label(resolved.friction.strategy)} 
                  to build momentum, framing our progress around {label(resolved.values.productivity_framing)}. 
                  Your path is clear."
                </p>
              </div>
              ...
              <div className="mt-10 pt-8 border-t border-stone-100">
                <button 
                  ...
                  className="w-full flex items-center justify-center gap-2 px-10 py-5 bg-stone-900 text-white rounded-2xl font-semibold hover:bg-stone-800 transition-all shadow-xl shadow-stone-200 active:scale-95"
                >
                  ...
                  >
                    <div className="text-center mb-8">
                      <div className="w-16 h-16 bg-emerald-100 rounded-2xl flex items-center justify-center mx-auto mb-4 text-emerald-600">
                        {/* Fix: User icon is now imported */}
                        <User size={32} />
                      </div>
                      <h3 className="text-2xl font-serif text-stone-800 mb-2">Final Step</h3>
                      <p className="text-stone-500 text-sm leading-relaxed">
                        To save your journey and sync your path, please connect with Google.
                      </p>
                    </div>
                    ...
                    <div className="space-y-4">
                       <button 
                        ...
                        className="w-full flex items-center justify-center gap-3 p-4 bg-white border border-stone-200 rounded-xl font-medium text-stone-700 hover:bg-stone-50 transition-all"
                       >
                         {isLoggingIn ? (
                       ...
                       <div className="p-4 bg-blue-50 rounded-xl border border-blue-100">
                         <div className="flex gap-3">
                           {/* Fix: Calendar icon is now imported */}
                           <Calendar size={18} className="text-blue-600 shrink-0" />
                           <div>
                             <p className="text-xs font-bold text-blue-900 mb-1">CALENDAR ACCESS REQUIRED</p>
                             <p className="text-[11px] text-blue-700 leading-relaxed">
                               Read-only access to your calendar allows Sensei to spot gaps in your schedule and suggest perfect timing for your paths.
                             </p>
                           </div>
                         </div>
                       </div>
                    ...
                    <button 
                      ...
                      className="mt-6 w-full text-stone-400 text-xs font-medium hover:text-stone-600 transition-colors"
                    >
                      Wait, go back
                    </button>
                  </motion.div>
  ...
  return (
    <div className="min-h-screen py-12 px-6 flex flex-col max-w-4xl mx-auto">
      ...
      <footer className="mt-12 flex items-center justify-between w-full">
        {currentStep > 0 && currentStep < 10 ? (
          <>
            <button onClick={prevStep} className="flex items-center gap-2 text-stone-400 hover:text-stone-600 font-medium transition-colors"><ChevronLeft size={20} />Back</button>
            <div className="flex gap-4">
              <button onClick={nextStep} className="text-stone-400 hover:text-stone-600 font-medium transition-colors">{isOptionalStep ? 'Skip section' : 'Skip'}</button>
              <button onClick={nextStep} className="flex items-center gap-2 px-8 py-3 bg-stone-900 text-white rounded-full font-medium shadow-lg shadow-stone-200">Next<ChevronRight size={20} /></button>
            </div>
          </>
        ) : null}
      </footer>
...
const SummaryItem: React.FC<{ icon: React.ReactNode; title: string; value: string }> = ({ icon, title, value }) => (
  <div className="flex gap-4">
    <div className="shrink-0 w-10 h-10 rounded-xl bg-stone-50 flex items-center justify-center text-stone-400">{icon}</div>
    <div>
